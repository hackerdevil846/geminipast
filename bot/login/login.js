...
async function startBot(loginWithEmail) {
	...
	let tooOldVersion;
	try {
		tooOldVersion = (await axios.get("https://raw.githubusercontent.com/ntkhang03/Goat-Bot-V2-Storage/main/tooOldVersions.txt", {
			timeout: 10000
		})).data || "0.0.0";
	} catch (err) {
		log.warn("VERSION CHECK", "Cannot check version, continuing...");
		tooOldVersion = "0.0.0";
	}
