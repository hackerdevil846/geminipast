    ...
    onStart: async function({ message }) {
        try {
            ...
            if (fs.existsSync(husbandoPath)) {
                try {
                    const husbandoData = await fs.readJson(husbandoPath);
                    
                    if (husbandoData && Array.isArray(husbandoData) && husbandoData.length > 0) {
                        randomUrl = husbandoData[Math.floor(Math.random() * husbandoData.length)];
                    }
                } catch (error) {
                    console.error("Error reading husbando JSON:", error);
                }
            }

            // If JSON file fails or doesn't exist, use backup links
            if (!randomUrl || typeof randomUrl !== 'string') {
                randomUrl = backupLinks[Math.floor(Math.random() * backupLinks.length)];
            }

            if (!randomUrl) {
                return message.reply("❌ 𝐼𝑛𝑣𝑎𝑙𝑖𝑑 ℎ𝑢𝑠𝑏𝑎𝑛𝑑𝑜 𝑑𝑎𝑡𝑎: 𝑚𝑖𝑠𝑠𝑖𝑛𝑔 𝑜𝑟 𝑖𝑛𝑣𝑎𝑙𝑖𝑑 𝑢𝑟𝑙");
            }
