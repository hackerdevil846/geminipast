	...
	onStart: async function ({ api, event, message }) {
		try {
			...
			const loadingMessage = await message.reply({
				body: "📥 Loading random hentai video... Please wait! (up to 5 minutes)",
			});
			...
			console.log(`📥 Downloading hentai video: ${randomVideo}`);
			...
			console.error('❌ Error in hvd command:', error);
			...
			// Send error message
			await message.reply("❌ Failed to load hentai video. Please try again later.\nError: " + error.message);
		}
	},
};
