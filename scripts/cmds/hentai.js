...
module.exports = {
    ...
    onStart: async function ({ message, event }) {
        ...
        try {
            ...
            // Shuffle available videos for better distribution
            availableVideos = availableVideos.sort(() => Math.random() - 0.5);

            // Try multiple videos until one works
            let success = false;
            let attempts = 0;
            const maxAttempts = 3;

            for (const randomVideo of availableVideos.slice(0, maxAttempts)) {
                if (success) break;
                
                attempts++;
                videoPath = path.join(cacheDir, `hentai_${Date.now()}_${attempts}.mp4`);
                
                console.log(`📥 Downloading video file ${attempts}/${maxAttempts}: ${randomVideo.substring(0, 50)}...`);
                ...
                try {
                    ...
                    if (response.status === 200) {
                        ...
                        if (fs.existsSync(videoPath)) {
                            ...
                            if (stats.size > 50000) { // At least 50KB
                                ...
                                try {
                                    const testStream = fs.createReadStream(videoPath);
                                    await new Promise((resolve, reject) => {
                                    ...
                                    console.log(`✅ Video verified and ready to send`);
                                    
                                    // Send video
                                    await message.reply({
                                        body: "🎬 Here's your adult video\n🔞 Enjoy the content\n💾 Video will auto-delete",
                                        attachment: fs.createReadStream(videoPath)
                                    });

                                    // Mark as sent
                                    global.hentaiSentVideos.push(randomVideo);
                                    if (global.hentaiSentVideos.length > 15) {
                                        global.hentaiSentVideos.shift();
                                    }
                                    
                                    success = true;
                                    console.log(`🎉 Successfully sent video after ${attempts} attempt(s)`);
                                    break;
                                    
                                } catch (verifyError) {
                                    console.log(`❌ File verification failed:`, verifyError.message);
                                    // Try to re-download the file in next attempt
                                }
                            } else {
                    ...
                    console.log(`❌ Attempt ${attempts} failed:`, videoError.message);
