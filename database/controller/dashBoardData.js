...
module.exports = async function (databaseType, dashBoardModel, fakeGraphql) {
	...
	global.db.allDashBoardData = DashBoardData;
		...
		try {
			...
			switch (mode) {
				case "create":
					...
					break;
				case "update":
					const oldData = global.db.allDashBoardData[index];
					...
					if (databaseType === "mongodb") {
						...
						global.db.allDashBoardData[index] = dataWillChange;
						writeJsonSync(pathDashBoardData, global.db.allDashBoardData, optionsWriteJSON);
				...
				case "remove":
					if (index != -1) {
						...
						else writeJsonSync(pathDashBoardData, global.db.allDashBoardData, optionsWriteJSON);
					}
					break;
			}
			return null;
	...
	async function create(userData) {
		return new Promise((resolve, reject) => {
			taskQueue.push(async () => {
				try {
					if (global.db.allDashBoardData.some(u => u.email == userData.email)) {
	...
	async function get(email) {
		return new Promise((resolve, reject) => {
			taskQueue.push(async () => {
				try {
					const data = global.db.allDashBoardData.find(u => u.email == email);
