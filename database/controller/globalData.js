...
module.exports = async function (databaseType, globalModel, fakeGraphql) {
	...
	global.db.globalData = Global;

	async function save(key, data, mode, path) {
		try {
			let index = _.findIndex(global.db.globalData, { key });
			if (index === -1 && mode === "update") {
				await create(key, {});
				index = _.findIndex(global.db.globalData, { key });
			}
			...
			switch (mode) {
				case "create":
					if (databaseType === "mongodb" || databaseType === "sqlite") {
						...
						global.db.globalData.push(dataCreated);
						return _.cloneDeep(dataCreated);
					} else if (databaseType === "json") {
						const newData = { key, data };
						global.db.globalData.push(newData);
						writeJsonSync(pathGlobalData, global.db.globalData, optionsWriteJSON);
						return _.cloneDeep(newData);
					}
					break;
				case "update":
					if (index === -1) return null;
					const oldData = global.db.globalData[index];
					...
					if (path && (typeof path === "string" || Array.isArray(path))) {
						...
						// Deep set logic handled below by merging
					}
					...
					if (databaseType === "mongodb") {
						...
						global.db.globalData[index] = dataUpdated;
						...
						global.db.globalData[index].data = finalData;
						writeJsonSync(pathGlobalData, global.db.globalData, optionsWriteJSON);
						return _.cloneDeep(global.db.globalData[index]);
					}
					break;
				case "remove":
					if (index != -1) {
						global.db.globalData.splice(index, 1);
						if (databaseType === "mongodb") await globalModel.deleteOne({ key });
						else if (databaseType === "sqlite") await globalModel.destroy({ where: { key } });
						else writeJsonSync(pathGlobalData, global.db.globalData, optionsWriteJSON);
					}
					break;
			}
			return null;
	...
	async function create(key, data) {
		return new Promise((resolve, reject) => {
			taskQueue.push(async () => {
				try {
					if (global.db.globalData.some(i => i.key == key)) {
						return resolve(_.cloneDeep(global.db.globalData.find(i => i.key == key)));
					}
	...
	async function get(key, path, defaultValue, query) {
		return new Promise((resolve, reject) => {
			taskQueue.push(async () => {
				try {
					let data = global.db.globalData.find(i => i.key == key);
